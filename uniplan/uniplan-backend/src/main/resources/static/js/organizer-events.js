/* =========================================================
   ORGANIZER EVENTS — mis eventos, filtros, cancelar
...
function renderTable(events) {
    ...
    if (!events.length) {
        tbody.innerHTML = `<tr><td colspan="8" class="ev-empty">No tienes eventos aún. <a href="/organizer/events/create" style="color:#a78bfa;">Crea uno →</a></td></tr>`;
        return;
    }
        ...
        <tr>
            ...
            <td style="display:flex; gap:0.4rem; flex-wrap:wrap; align-items:center;">
                ...
                <button class="ev-btn ev-btn-edit"
                        onclick="location.href='/organizer/events/edit?id=${e.id}'">Editar</button>
                ${e.status === "ACTIVE"
...
async function cancelEvent(id, btn) {
    if (!confirm("¿Cancelar este evento? Los estudiantes inscritos serán notificados.")) return;
